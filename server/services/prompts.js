...
export function buildRenderPrompt({ scheme, customColors, landscape, decor, decorColor, serviceType = 'permanent', userPrompt }) {
  const custom = sanitizeUserPrompt(userPrompt);
  if (custom) return buildFromUserPrompt(custom, false);

  const colorText = describeColors(scheme, customColors);
  ...
  const isBrightDim = scheme === 'bright-dim-1-3';
  const rooflineInstructions = isChristmas
    ? CHRISTMAS_FASCIA
    : isPermanent
  ...
  lines.push(
    isCoolWhite
      ? 'Shift only the ambient light to a natural blue-hour evening. Use crisp neutral/cool white LEDs around 5000K with clean white wall wash; absolutely no amber, yellow, or warm tint in the exterior lights. Keep any existing window light subtle.'
      : isBrightDim && (brightDimIsDark || brightDimIsCustomColor)
        ? 'Shift ambient light to early-evening dusk (deep blue sky) so the bulbs read clearly. Keep any existing window light subtle. The ADDED roofline LED color must follow the COLOR LOCK above — do not warm-tint those LEDs to amber or warm white.'
        : 'Shift lighting to early-evening dusk (deep blue sky) so the bulbs read clearly. Keep any existing window light subtle. Natural realistic lighting — not neon, not cartoon.',
  ...
  if (landscape) {
    ...
    lines.push('Do NOT add landscape lighting, path lights, or change the garden beds. Roofline bulbs only.');
  }
...
export function buildShortPrompt({ scheme, customColors, landscape, decor, decorColor, serviceType = 'permanent', userPrompt }) {
  const custom = sanitizeUserPrompt(userPrompt);
  if (custom) return buildFromUserPrompt(custom, true);

  const colorText = shortColor(scheme, customColors);
  const isPermanent = serviceType === 'permanent';
  const isChristmas = serviceType === 'christmas' || scheme === 'christmas';
  const isBrightDim = scheme === 'bright-dim-1-3';
  const bits = [
    'exact same house photo, same garden and plants unchanged',
    isChristmas
    ...
    scheme === 'cool-white'
      ? 'natural blue-hour sky, 5000K crisp white exterior lighting, clean white wall wash, no amber or yellow tint, photorealistic'
      : 'natural dusk sky, subtle existing windows, photorealistic',
  ];
